...
public class ApproveModel extends BaseHttpAsyncModel<ApproveBean> {

    public ApproveModel(int model_id, Context context, ModelCompleteCallback<BaseResponesBean> callback) {
    ...
    private BaseResponesBean getDatas() {
        String url = BaseApplication.url + "/integrationLeaderApprove/doAddSubtractScore";
        List<NameValuePair> parmas = new ArrayList<>();
        parmas.add(new BasicNameValuePair("token", BaseApplication.token));
        parmas.add(new BasicNameValuePair("model.approve_id", holder.getApprove_id()));
        parmas.add(new BasicNameValuePair("model.type", holder.getType()));
        parmas.add(new BasicNameValuePair("model.score", holder.getScore()));
        parmas.add(new BasicNameValuePair("model.reason", holder.getReason()));
        parmas.add(new BasicNameValuePair("model.veto_id", holder.getVeto_id()));
