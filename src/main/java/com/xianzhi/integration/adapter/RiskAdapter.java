...
public class RiskAdapter extends MyBaseAdapter<AddressTypeBean.ListBean> {

    private OnRiskClickListener listener;

    public RiskAdapter(OnRiskClickListener listener, Context context, List datas) {
        super(context, datas);
    ...
    public View getViewDatas(final int position, View convertView, ViewGroup parent) {
        ...
        holderView.tv_content.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                listener.onRiskClick(datas.get(position).getName(), datas.get(position).getPid());
            }
        });
